...
#### 可行性前置判斷（2026-09-20 實測後補）

單次探測推出的區間寬度是 `1 + RTT`，所以**收斂下限就是 RTT，不確定度下限是 RTT/2**。
一旦 `RTT/2` 超過 `MAX_USEFUL_UNCERTAINTY_SECONDS`，再打幾次探測都不可能產出
會被接受的結果 —— 而這件事在拿到第一個 RTT 樣本時就算得出來。

因此第一次探測後立刻判斷，超標就放棄，記 log 並退回 NTP。

**實測結論：中正這個站不適用這個機制。** 兩次實跑量到的回應時間穩定在
4.0～5.4 秒（`Server=cloudflare`），不確定度下限 2.15 秒，是門檻 0.15 秒的 14 倍。
log 裡可以直接看到探測 2～5 次的區間寬度完全沒有變動 —— 因為每次探測給出的
約束（寬 5.3 秒）比已經累積的區間（寬 2.32 秒）還模糊，交集不會narrow。

**改打輕量端點沒有用。** 靜態資源的 `Date` 是 Cloudflare 邊緣節點蓋的，而
Cloudflare 本來就與標準時間同步到毫秒 —— 等於繞一圈回到 NTP，量不到真正決定
「時段開了沒」的源站。要量源站就得打動態頁，動態頁就是 4 秒。這個死結解不開。

機制本身保留：它在回應快的站上仍然正確（見第 9 節的離線收斂模擬），
只是中正不符合前提。
...
dev mode 走的是**與正式搶場地完全相同的流程**，包含 NTP 校時、伺服器時鐘探測與
事後偵察。差別只有兩點：開搶時刻與預約時段由使用者直接輸入，不從 `BOOKING_WEEKDAY`
推導；以及沒有手動毫秒偏移，因此儀表的基準直接用輸入的時刻。

**這是有意識的取捨。** dev mode 打的是真實網站，每次執行會多送約 11 個請求
（原本只有 2 個），成本明顯提高。換到的是：dev mode 成為一次真正的彩排，
送出時刻補償（`θ − RTT/2 + margin`）這條路徑能被實際驗證 —— 若 dev mode 跳過校時，
`θ` 恆為 0，整個補償邏輯在實站上就從來沒有被執行過，直到週四真正開搶的那一刻。

因此 **dev mode 不再適合隨手跑**：它會真的送出 `StepFlag=25` 訂場地，也會產生
完整的探測流量。使用時請遵守第 8 節的請求預算精神 —— 一次跑完整流程，
不要短時間內反覆執行。

探測要收斂需要 T−3min 到 T−30s 約 2.5 分鐘的跑道，所以 dev mode 的開搶時刻
**建議指定 4 分鐘以上之後**。指定得太近不會出錯：`run_with_deadline` 拿到的預算
若為零或負數就完全跳過探測、一個請求都不送；預算太短則只跑得完幾次探測、
無法收斂到 `MAX_USEFUL_UNCERTAINTY_SECONDS` 以內，自動退回 NTP。兩種情況都會在
log 中留下 `探測 N 次` 與 `量測失敗` 的紀錄。
